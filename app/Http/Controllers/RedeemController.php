...
class RedeemController extends KbvController
{
    ...
    public function detail($id)
    {
        ...
        $img    = $mess->media_id ? Media::find($mess->media_id)->source : url('images/portrait/medium/avatar-m-4.png');

        $redeem->sumary = $mess->content;
        $redeem->title  = $mess->title;
        $redeem->media  = $img;
        return view('.redeem.detail', compact('redeem'));
    }
    ...
    function redeems()
    {
        ...
        return view('.redeem', compact('redeems', 'r', 'query', 'search'));
    }
